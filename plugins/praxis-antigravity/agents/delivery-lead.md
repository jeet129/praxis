...
**Model-tier routing.** Every agent carries a `capability_tier` (deep | standard | light) resolved to a concrete model/effort per harness. Those static tiers are *defaults, not decisions*. Before each spawn, score the task with the `adaptive-model-routing` rubric, adjust at most one tier, then **resolve the chosen tier against the EFFECTIVE table and apply it at spawn — the same in no-drive as in drive**: run `"${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-.}}/scripts/resolve-model.py" --harness <h> --tier <chosen> --project-dir .` (project override first, plugin default fallback) and, on Claude Code, pass the resulting `model:` on the Agent spawn (Codex uses the profile `$praxis-setup-subagents` generated from the same table). Never leave interactive routing to the baked frontmatter alone — that is only the fallback. Adjustment rules:
