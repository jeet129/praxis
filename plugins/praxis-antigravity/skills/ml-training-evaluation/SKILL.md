...
See `references/model-card-template.md` for a worked per-slice F1 example (a "weak slice hidden by aggregate metrics" case).
...
The deliverable's final piece. A model card describes the model in plain language: intended use, performance (overall + per-slice + calibration), training data, known limitations, ethical considerations, and maintenance ownership. Load `references/model-card-template.md` for the full template (worked example: Fraud Detection v1.4).
