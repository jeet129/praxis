...
Per the NFR register, the model's prediction is one component of a larger user-facing operation. See `references/latency-budget.md` for a worked 200ms budget breakdown.
...
Distinct from code-deployment strategies (per `deploy-release`). ML rollouts add model-specific patterns: shadow deployment (log predictions without serving), A/B testing (split traffic, measure business impact), canary deployment (staged traffic ramp), multi-armed bandits (dynamic traffic allocation), and progressive delivery (canary + feature flags — the modern default for high-stakes releases).

Load `references/rollout-strategies.md` for shadow deployment mechanics, A/B testing requirements, canary staging, bandit use criteria, and the progressive delivery pattern.
