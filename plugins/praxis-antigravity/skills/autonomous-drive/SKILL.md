...
One iteration, one task. This SKILL is what the Delivery Lead runs inside a drive session — the autonomy dial (`.project/governance/autonomy.yaml` when the project carries an override, else the plugin's `governance/autonomy.yaml`) sets how often a human is consulted; this SKILL is what happens between those touchpoints. No contract, no loop: every claim here traces back to `references/loop-contracts.md`.
