...
After either install, **restart the tool (start a new session)** so its commands, agents, skills, and hooks load. On Claude Code, `/reload-skills` and `/reload-plugins` hot-reload skills/agents mid-session, but a fresh session is the reliable way to fully activate a newly installed plugin (a reload doesn't rebuild the slash-command index).
