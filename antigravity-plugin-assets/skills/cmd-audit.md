...
Interview me about what feels hard. Run static analysis (complexity / duplication / coverage) with output captured to `.project/working/static-analysis.log` — consume the top-N summary per dimension (worst 20 files by complexity, duplication clusters, coverage gaps), never the full tool dump. Produce triaged register at `.project/operational/debt-register.md`. Limit to 20-50 items. Classify each per Fowler's quadrants (prudent/reckless x intentional/inadvertent). Prioritize by payoff x risk x cost.
